...
class PrimaryPageController extends ScrollController {
  ...
  double? get page {
    ...
    final PrimaryPagePosition position = this.position as PrimaryPagePosition;
  ...
  void attach(ScrollPosition position) {
    ...
    final ScrollPosition pagePosition = position;
    if (position is PrimaryPagePosition) {
      pagePosition as PrimaryPagePosition;
...
class PrimaryPagePosition extends ScrollPosition implements PageMetrics, ScrollActivityDelegate {
  PrimaryPagePosition({
  ...
  void absorb(ScrollPosition other) {
    ...
    final PrimaryPagePosition typedOther = other as PrimaryPagePosition;
...
class PrimaryPageCoordinator implements ScrollActivityDelegate, ScrollHoldController {
  ...
  PrimaryPageCoordinator(PrimaryPageController selfController, PrimaryPageController parentController) {
    ...
    if (!(_outerController?.childPageController.contains(_selfController) ?? true)) {
...
//        (getInnerController().position as PrimaryPagePosition);
    PrimaryPagePosition? outPosition = isOuterControllerEnable() ? (getOuterController()?.position as PrimaryPagePosition?) : null;
...
//    }
    ...
    if (outPosition != null && (outPosition.pixels >= outPosition.minScrollExtent && outPosition.pixels <= outPosition.maxScrollExtent)) {
//      if (outPosition.pixels % outPosition.viewportDimension != 0) {
//        outPosition.applyUserOffset(delta);
//        return;
//      }

      if (getOuterController() != null && getOuterController()!.childPageController.length > outPosition.page!.round()) {
        PrimaryPageController currentChildPageController = getOuterController()!.childPageController[outPosition.page!.round()];

        PrimaryPagePosition currentInnerPosition = currentChildPageController.position as PrimaryPagePosition;

        if (delta < 0 ? currentInnerPosition.pixels < currentInnerPosition.maxScrollExtent : currentInnerPosition.pixels > currentInnerPosition.minScrollExtent) {
          isOperateBody = true;
  ...
  void goBallistic(double velocity) {
    ...
    if (outPosition != null && getOuterController()!.childPageController.length > outPosition.page!.round()) {
      PrimaryPageController currentChildPageController = getOuterController()!.childPageController[outPosition.page!.round()];

      PrimaryPagePosition currentInnerPosition = currentChildPageController.position as PrimaryPagePosition;

//    if (isOperateBody) {
      if (!isOperateBody && (outPosition != null) && (outPosition.pixels > outPosition.minScrollExtent && outPosition.pixels < outPosition.maxScrollExtent)) {
      ...
      if (isOperateBody && velocity > 0) {
        if (currentInnerPosition.pixels < currentInnerPosition.maxScrollExtent && currentInnerPosition.pixels > currentInnerPosition.minScrollExtent) {
          outPosition.goBallistic(0.0);
  ...
  ScrollHoldController hold(VoidCallback holdCancelCallback) {
    beginActivity(HoldScrollActivity(delegate: this, onHoldCanceled: holdCancelCallback), HoldScrollActivity(delegate: this, onHoldCanceled: holdCancelCallback));
  ...
  Drag drag(DragStartDetails details, VoidCallback dragCancelCallback) {
    ...
    beginActivity(DragScrollActivity(this, drag), DragScrollActivity(this, drag));
  ...
  void beginActivity(ScrollActivity? newOuterActivity, ScrollActivity? newInnerActivity) {
    ...
    if ((newOuterActivity == null || !newOuterActivity.isScrolling) && (newInnerActivity == null || !newInnerActivity.isScrolling)) {
...
class _PageViewState extends State<PrimaryPageView> {
  ...
  Widget build(BuildContext context) {
    ...
    final ScrollController? scrollController = widget.primary ? PrimaryScrollController.of(context) : widget.controller;

    if (scrollController != null && widget.primary && scrollController is PrimaryPageController) {
